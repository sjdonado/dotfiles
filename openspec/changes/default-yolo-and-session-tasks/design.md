...
5. **Verification gets a name, for the same reason review has one.** `adversarial-review` is a named skill, so its spend is visible: two reviewers, once per round, at the end of a run. Verification is the other mechanical half and had no name, so it landed in general-purpose subagents indistinguishable from anything else, and nobody could say what running the checks costs. A `verification` skill fixes that and is the obvious cheap-tier candidate: a settled contract, a machine-checkable oracle, and no judgement call about success. Its constraints are what keep a cheap tier safe, namely that it repairs within scope, never redesigns, never pushes or opens a pull request, reports unrelated failures instead of absorbing them, and reports a rung it could not run as unverified rather than passing. The review half is deliberately not pushed downward the same way: a reviewer too weak to find the bug is not a saving.

6. **Delegate work, never waiting.** Observed in this change's own bench run: a subagent asked to run a long benchmark and report spent about 80k tokens polling and returned no result, and stopping it killed the paid run it owned. A long-running command belongs in the session that can see it through.

7. **Cost accounting includes the subagents a workflow already spawns.** `adversarial-review` dispatches up to two reviewers per round, at the orchestrator's tier unless told otherwise. Any guidance about delegation that ignores them understates a run's cost by the most expensive part of its tail. The instruction says to count them; it does not prescribe a tier for them, because a reviewer that is too weak to find the bug is not a saving.

8. **Acceptance is declared, not assumed.** The task list ends with the behavioral batch and states plainly that local checks alone do not validate an instruction change. Because the budget is shared with the unarchived `improve-harness-effectiveness`, the first task is a decision for the human: authorize a combined batch that closes both, or accept this change as unvalidated and say so in the PR. The agent does not spend paid sessions on its own initiative.
