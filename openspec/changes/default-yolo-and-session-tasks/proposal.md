...
- Name the verification half: a `verification` skill, the counterpart to `adversarial-review`, that runs the oracle ladder in a subagent, fixes what the change broke, and reports a verdict. Named rather than general-purpose so its token cost is attributable, and the natural candidate for a cheaper tier.
...
Edits `agents/AGENTS.md` (the routing table, the planning section, and the continuity section), and the `yolo`, `proto`, `land`, and `handoff` skills under `agents/skills/`. Adds `agents/skills/verification/`. Adds a session task ledger file convention under the already-ignored `.agent/` directory.
