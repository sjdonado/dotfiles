...
#
# The TTT tab is keyed by the label herdr kept across the restore, which is also what
# tells a restored pane apart from a shell someone opened themselves: only a plugin
# pane carries one.
set -euo pipefail
...
# ttt-tab's [[panes]] title, and the only plugin pane left to repair.
[ "$label" = ttt ] || exit 0
...
"$herdr_bin" pane run "$pane" ttt >/dev/null 2>&1 || true
