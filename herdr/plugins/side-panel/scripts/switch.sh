...
# herdr runs plugin commands with the server's own PATH, which is whatever
# started the server. On a remote box that is often a system PATH without
# ~/.local/bin, so a bare `herdr` is not found and every call dies with 127.
# herdr exports HERDR_BIN_PATH for exactly this; herdr-lazygit resolves it the
# same way.
...
if [ -z "$work_pane" ] || [ -n "$focused_label" ]; then
  work_pane=$("$herdr_bin" pane list 2>/dev/null \
...
pane_cwd=$("$herdr_bin" pane list 2>/dev/null \
...
close_label() {
  panes_for "$1" | while read -r pane_id; do
    [ -n "$pane_id" ] && "$herdr_bin" pane close "$pane_id" >/dev/null 2>&1 || true
  done
}
...
if [ "$TARGET" = "lazygit" ]; then
  exec "$herdr_bin" plugin action invoke open --plugin herdr-lazygit
fi
...
pane_id=$("$herdr_bin" pane split --pane "$work_pane" --direction right --cwd "$root" --focus 2>/dev/null \
...
"$herdr_bin" pane rename "$pane_id" "$own_label" >/dev/null 2>&1 || true
exec "$herdr_bin" pane run "$pane_id" "exec $panel_cmd"
