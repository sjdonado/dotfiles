...
| route-open-shape | 1 | open shape, no workflow named: the cheap route builds a slice and stops, no invented blank-input fallback, task list created, ignored and ticked |
| route-bounded | 1 | fully specified ticket: the expensive route lands both stated behaviors in one pass, task list created, ignored, untracked and ticked, note points at it |
