...
Triage a Linear ticket. This is analysis ONLY — do NOT write, edit, run, or commit anything.
...
2. Supplement via Linear MCP only if the input is just an ID/URL (no `<issue>` block): `linear_get_issue` with `includeRelations: true`. The MCP omits video transcripts and comment threads, so prefer pasted content when both exist.
...
5. Output:
   ...
   - **Options** — 1-4 approaches. For each: what changes, **Pros**, **Cons**, risk/blast radius.
